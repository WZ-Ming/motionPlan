...
void motionPlan::ini_path_data(const pathInitData &pathInit)
{
    ...
    deltaT=fabs(pathInit.deltaT); maxProportion=fabs(pathInit.maxProportion); dec_jerk_muti=fabs(pathInit.dec_acc_muti);
...
bool motionPlan::get_fixed_spd(double &fixedVe,double &fixedVmax)//获得修正的末速度
{
    ...
    pathInitTmp.cmd_pos=cmd_pos; pathInitTmp.v0=v0; pathInitTmp.VMax=Vmax; pathInitTmp.ve=ve;
    pathInitTmp.Jerk=Jerk; pathInitTmp.max_acc=maxAcc;
    pathInitTmp.deltaT=deltaT; pathInitTmp.dec_acc_muti=dec_jerk_muti; pathInitTmp.maxProportion=maxProportion;
    fixedVmax=0;
    while(path_busy){
        doMotionPlan(can_do_path);
