...
void sendThread::initClient()
{
    ...
    tcpClient->setReadBufferSize(recThread->bufferSize);
...
void sendThread::onNewConnection()
{
    ...
    tcpSocket->setReadBufferSize(recThread->bufferSize);
    ...
    case fileSendBtn:{
        ...
        if(file.open(QIODevice::ReadOnly)){
            ...
            while(sendFlag){
                ...
                str=file.read(recThread->bufferSize);
...
void sendThread::onServerReadyRead()
{
    ...
    while(str.size()>recThread->bufferSize){
        recThread->byteQueue->enqueue(str.mid(0,recThread->bufferSize));
        str=str.mid(recThread->bufferSize,str.size()-recThread->bufferSize);
    }
